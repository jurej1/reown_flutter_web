...
class UtilsJS {
  static Object? dartify(JSAny? jsObject, {bool deep = true}) {
    ...
    if (jsObject.isA<JSBigInt>()) return (jsObject as JSBigInt).toDart;
    if (jsObject.isA<JSArray>()) return (jsObject as JSArray).toDartDynamicList;
    if (jsObject.isA<JSObject>()) {
      return deep ? (jsObject as JSObject).toMap() : jsObject;
    }
